...
| 🪟 独立毛玻璃面板 | **【全新】**舍弃 NSPopover 改用独立 NSWindow，集成 Cocoa 原生三色交通灯，支持背景拖拽与任意大小拉伸，支持状态栏/屏幕中央双模弹出 |
| 📡 空间物理定位雷达 | **【全新】**环形物理分布探测仪表盘与罗盘度盘，反向三角几何方位逆旋转实时锁向，支持全向键盘行走模拟、偏角微调与无级变焦 |
...
## 📡 空间物理定位雷达与毛玻璃浮动面板 (v1.9.3 新增)

STATUS CTRL v1.9.3 迎来重磅跨代升级，带来底层呈现架构与硬件感知交互的全新篇章：

- **🪟 独立毛玻璃面板与自由拉伸窗口 (Translucent Resizable NSWindow)**：
  - **舍弃 Popover 局限**：彻底舍弃传统临时气泡，改用高度定制的独立 `NSWindow` 承载仪表盘面板，集成标准左上角原生三色交通灯。
  - **透明标题栏与玻璃拟态 (Glassmorphism)**：融合 HUD 极客毛玻璃视觉材料（`.hudWindow` + `.behindWindow`），支持背景拖拽和任意大小拉伸，自适应任何高宽尺寸。
  - **双模开辟体验**：左键单击状态栏图标在正下方弹出对齐的悬浮窗口；右键菜单选择“打开主面板”则在屏幕中央打开完全脱离状态栏的独立中央主面板。

- **📡 空间物理定位雷达与无线连接拆分 (Spatial Telemetry Radar Dashboard)**：
  - **UI 模块化物理拆分**：将无线页面拆分为两个独立的二级导航子页面：`📡 空间物理定位雷达` 和 `⚡ 流量监控与高可用测速`。
  - **罗盘度盘与反向三角几何**：雷达圆盘外围集成精致的罗盘度盘（N, S, E, W & 刻度），应用反向三角几何变换公式，当用户移动时，信号源在雷达图上的物理相对方位和测距能够实时重算并绝对锁定，实现高精度罗盘对齐效果。

- **⌨️ 全向键盘步进与航向微调 (Spatial Compass Keys)**：
  - 支持键盘按 `W / A / S / D` 或 `↑ / ↓ / ← / →` 方向键模拟您的物理行走轨迹，实时更新坐标和物理距离。
  - 支持按 `Q / E` 微调电脑朝向偏角，按 `R` 一键重置物理位移与航向归零，按 `+ / -` 实现雷达图焦距无级变焦。

- **⚡ 系统级加固与防崩审计 (Stability Audits)**：
  - **死锁防御 (Deadlock Prevention)**：全面审计 SMC 传感器寄存器锁释放路径，通过 defer 机制加固，避免并发读取硬件指标时的阻塞与死锁。
  - **零 Force Unwraps 加固**：清理内存深度净化和垃圾文件解析模块彻底清空崩溃隐患，确保所有路径防空安全。
